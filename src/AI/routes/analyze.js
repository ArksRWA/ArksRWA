...
router.post('/analyze-company', async (req, res) => {
  try {
    ...
    // Extract web scraping sources information
...
router.get('/analyze-company/sources-demo', async (req, res) => {
  try {
    ...
    const result = await fraudAnalyzer.analyzeCompany(legitimateCompany);
    const dataSources = router.extractDataSources(result);
    ...
    const suspiciousResult = await fraudAnalyzer.analyzeCompany(suspiciousCompany);
